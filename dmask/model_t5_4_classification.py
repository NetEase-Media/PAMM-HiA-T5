...
class T5Attention(nn.Module):
    ...
    def forward(
        ...
        weights_ori = F.softmax(scores.float(), dim=-1).type_as(scores)  # (bs, n_heads, qlen, klen)
        weights = F.dropout(weights_ori, p=self.dropout, training=self.training)  # (bs, n_heads, qlen, klen)
        ...
        if self.output_attentions:
            outputs = outputs + (weights_ori,)
        if self.has_relative_attention_bias:
            outputs = outputs + (position_bias,)
        return outputs   #  (context,present_key_value_state,weights,position_bias) weights:(bs, n_heads, qlen, klen)
...
class T5LayerSelfAttention(nn.Module):
    ...
    def forward(
        ...
        attention_output = self.SelfAttention(
            ...
            use_cache=use_cache,
        )#  (context,present_key_value_state,weights,position_bias) weights:(bs, n_heads, qlen, klen)
        y = attention_output[0]

        if self.is_decoder and decoder_paths_mask != None:

            attention_weights = attention_output[2]

            decoder_paths_mask = decoder_paths_mask.unsqueeze(1).repeat(1, attention_weights.shape[1], 1, 1)
            paths_mask_weights = decoder_paths_mask * attention_weights  # torch.matul(decoder_paths_mask,attention_weights)   #(bs, n_heads, qlen, qlen)
            paths_score_sum = torch.sum(paths_mask_weights, -1)
            goal_score_sum = torch.ones(paths_score_sum.shape).cuda()
            #   todo  .....mean loss=[0,1]  sum loss=[0,B]
            gate_loss = torch.mean(goal_score_sum - paths_score_sum)
        else:
            gate_loss = 0
            
        layer_output = hidden_states + self.dropout(y)

        outputs = (layer_output,) + attention_output[1:]+(gate_loss,)  # add attentions if we output them
        return outputs
...
class T5Block(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False):
        super().__init__()
        self.is_decoder = config.is_decoder
        self.gate_layer=nn.Linear(768,1)
    ...
    def forward(
        ...
        self_attn_loss=self_attention_outputs[-1]
        ...
        outputs = outputs + (present_key_value_state,) + attention_outputs+(self_attn_loss,)
...
class T5Stack(T5PreTrainedModel):
    ...
    def forward(
        ...
        for i, (layer_module, past_key_value_state) in enumerate(zip(self.block, past_key_value_states)):
            ...
            self_attn_loss+=layer_outputs[-1]
        ...
        if self.output_attentions:
            outputs = outputs + (all_attentions,)+(self_attn_loss,)
...
class T5Model(T5PreTrainedModel):
    ...
    Examples::
        ...
        return decoder_outputs + encoder_outputs + (decoder_outputs[-1],)
